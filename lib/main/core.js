...
module.exports = (function() {
	...
	var Client = function(direttoClient, serviceUri) {
		...
		var fetchMetaDataDocuments = function(documents, callback){
			direttoClient.r.postJSON({
				uri: uri.documents()+"/multiple",
				json : {
					documents : documents
				},
				auth : direttoClient.auth
		...
		var fetchDocumentSnapshots = function(documents, callback){
			direttoClient.r.postJSON({
				uri: uri.documents()+"/snapshot",
				json : {
					documents : documents
				},
				auth : direttoClient.auth
		...
		var fetchFullDocuments = function(documents, callback){
			direttoClient.r.postJSON({
				uri: uri.documents()+"/full",
				json : {
					documents : documents
				},
				auth : direttoClient.auth
		...
		var dispatchQuery = function(query, callback){
			direttoClient.r.post({
				uri: uri.queryDispatch(),
				json : {
					query : query
				},
				auth : direttoClient.auth
		...
		var forwardQuery = function(queryUri, callback){
			direttoClient.r.get({
				uri: queryUri,
				auth : direttoClient.auth,
				...
				else if(response && response.statusCode && (response.statusCode !== 204 && response.statusCode !== 303)){
		...
		var getResultPage = function(pageUri, callback){
			genericAuthedGet(pageUri, callback);
		}
		...
		var resolveTags = function(tags, callback){
			direttoClient.r.postJSON({
				uri: uri.basetags()+"/multiple",
				json : {
					values : tags
				},
				auth : direttoClient.auth
		...
		return {
			...
			fetchMetaDataDocuments : fetchMetaDataDocuments,
			fetchDocumentSnapshots : fetchDocumentSnapshots,
			fetchFullDocuments : fetchFullDocuments,
